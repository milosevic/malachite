...
/// A validator that equivocates across a prune must still be caught.
///
/// The realistic path, driven entirely through the public API with default thresholds:
/// validators vote in round 1, consensus moves on and prunes the old tallies (what the
/// driver does once a later round decides), and then a conflicting vote for round 1
/// arrives late — through sync, WAL replay, or simply a slow link. `prune_votes` drops
/// `votes_by_address` along with the tallies, so the keeper no longer remembers the first
/// vote and treats the conflicting one as a first vote: no evidence is recorded, and the
/// pruned round's tally is silently rebuilt from it.
///
/// Generated by Studio to reproduce obs:equivocation_missed_after_prune, which it
/// classified `data-loss-or-corruption` and flagged fix-first. The `#[ignore]` it carried
/// ("fails on current code") is gone because the code was fixed: the first-vote record now
/// lives outside the pruned region, so a conflicting vote arriving after its round was
/// pruned is still recorded as evidence. Ledger F-28.
#[test]
fn equivocation_across_a_prune_is_still_recorded_as_evidence() {
    let (a, mut k) = setup([1, 1, 1, 1, 1, 1]);

    // Round 1: three validators vote for value 4.
    for i in 0..3 {
        k.apply_vote(vote_for(1, val(4), a[i]));
    }
    assert!(k.has_vote_quorum(Round::new(1), &ValueId::new(4)));

    // Consensus advances and the driver prunes everything below round 6.
    k.prune_votes(Round::new(6));

    // A late, CONFLICTING vote from a validator that already voted in round 1.
    let out = k.apply_vote(vote_for(1, val(9), a[0]));

    assert_eq!(
        k.evidence().len(),
        1,
        "a second, conflicting vote from a validator already counted in round 1 must be \
         recorded as evidence even after that round's tallies were pruned"
    );
    assert!(
        out.is_empty(),
        "an equivocating vote is never tallied, so it can report no threshold"
    );
}
